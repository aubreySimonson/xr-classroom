...
namespace XRMultiplayer
{
    ...
    public class XRHandPoseReplicator : NetworkBehaviour
    {
        ...
        XROrigin xrOrigin;
        XRInputModalityManager.InputMode currentTrackingType;

        Transform leftTrackedHandPosition;
        Transform rightTrackedHandPosition;

        Transform leftControllerPosition;
        Transform rightControllerPosition;
        ...
        {
            base.OnDestroy();
            if (IsOwner && xrModalityManager != null)
            {
        ...
        public override void OnNetworkSpawn()
        {
            ...
            if (IsOwner)
            {
                ...
                if (xrModalityManager == null && xrOrigin != null)
                    xrOrigin.TryGetComponent(out xrModalityManager);
        ...
        {
            currentTrackingType = current;
            ChangeControllerType(current);
        }

        void SetupLocalHands()
        {
            if (leftControllerPosition == null && xrModalityManager.leftController != null)
                leftControllerPosition = xrModalityManager.leftController.transform;
            if (rightControllerPosition == null && xrModalityManager.rightController != null)
                rightControllerPosition = xrModalityManager.rightController.transform;

            if (xrModalityManager.leftHand == null)    //Rig doesn't have hands setup
            {
                if (leftTrackedHandPosition == null)
                    leftTrackedHandPosition = leftControllerPosition;
                if (rightTrackedHandPosition == null)
                    rightTrackedHandPosition = rightControllerPosition;

                SetTrackingType(XRInputModalityManager.InputMode.MotionController);
            }
            else    //Setup Hands and modality change listeners
            {
                var leftHandSkeleton = xrModalityManager.leftHand.GetComponentInChildren<XRHandSkeletonDriver>();
                var rightHandSkeleton = xrModalityManager.rightHand.GetComponentInChildren<XRHandSkeletonDriver>();

                if (leftTrackedHandPosition == null && leftHandSkeleton != null)
                    leftTrackedHandPosition = leftHandSkeleton.rootTransform;

                if (rightTrackedHandPosition == null && rightHandSkeleton != null)
        ...
        {
            XRInputModalityManager modalityMangager = xrModalityManager;
            if (modalityMangager == null)
                modalityMangager = FindFirstObjectByType<XRInputModalityManager>();

            // Early out if hands are not setup
            if (modalityMangager == null || modalityMangager.leftHand == null)
            {
            ...
            foreach (var fingerSync in m_LocalHandFidelityOptions[0].fingerJoints)
            {
                ...
                {
                    m_FingerRotationsLeft.Add(joint.jointTransform.localEulerAngles);
                }
            }
            ...
            foreach (var fingerSync in m_LocalHandFidelityOptions[1].fingerJoints)
            {
                foreach (var joint in fingerSync.jointTransformReferences)
                {
                    m_FingerRotationsRight.Add(joint.jointTransform.localEulerAngles);
                }
            }
            ...
            for (int i = 0; i < m_LocalHandFidelityOptions[0].fingerJoints.Length; i++)
            {
                for (int j = 0; j < m_LocalHandFidelityOptions[0].fingerJoints[i].jointTransformReferences.Count; j++)
                {
                    m_FingerRotationsLeft[currentIdx++] =
                        m_LocalHandFidelityOptions[0].fingerJoints[i].jointTransformReferences[j].jointTransform.localEulerAngles;
            ...
            for (int i = 0; i < m_LocalHandFidelityOptions[1].fingerJoints.Length; i++)
            {
                for (int j = 0; j < m_LocalHandFidelityOptions[1].fingerJoints[i].jointTransformReferences.Count; j++)
                {
                    m_FingerRotationsRight[currentIdx++] =
                        m_LocalHandFidelityOptions[1].fingerJoints[i].jointTransformReferences[j].jointTransform.localEulerAngles;
                }
            }
        }
        ...
        void GetNetworkFingerRotations()
        {
            ...
            for (int i = 0; i < m_HandCurler[hand].handFidelityOptions[0].fingerJoints.Length; i++)
            {
                ...
                {
                    if (currentIdx < m_FingerRotationsLeft.Count)
                    {
                        m_HandCurler[hand].handFidelityOptions[0].fingerJoints[i].jointTransformReferences[j].jointTransform.localRotation =
                            Quaternion.Slerp(m_HandCurler[hand].handFidelityOptions[0].fingerJoints[i].jointTransformReferences[j].jointTransform.localRotation,
            ...
            for (int i = 0; i < m_HandCurler[hand].handFidelityOptions[0].fingerJoints.Length; i++)
            {
                for (int j = 0; j < m_HandCurler[hand].handFidelityOptions[0].fingerJoints[i].jointTransformReferences.Count; j++)
                {
                    if (currentIdx < m_FingerRotationsRight.Count)
                    {
                        m_HandCurler[hand].handFidelityOptions[0].fingerJoints[i].jointTransformReferences[j].jointTransform.localRotation =
                           Quaternion.Slerp(m_HandCurler[hand].handFidelityOptions[0].fingerJoints[i].jointTransformReferences[j].jointTransform.localRotation,
        ...
        public void SetTrackingType(XRInputModalityManager.InputMode trackingType)
        {
            ...
            if (XRINetworkPlayer.LocalPlayer != null)
                XRINetworkPlayer.LocalPlayer.SetHandOrigins(activeLeftHandPosition, activeRightHandPosition);
        }
